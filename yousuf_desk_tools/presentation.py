...
class Slide:
    ...
    def __init__(self, title="", content="", bg_color=DEFAULT_SLIDE_BG, image_path=""):
        self.title = title
...
class SlideshowWindow(QWidget):
    ...
    def __init__(self, slides, parent=None):
        ...
        self.setup_ui()
        self.showFullScreen()
        # Position controls after showing fullscreen
    ...
    def setup_ui(self):
        ...
        slide_layout.addWidget(self.image_label)
    ...
    def position_controls(self):
        """Position controls at bottom of screen after fullscreen is established"""
        self.controls.move(0, self.height() - 80)
        self.controls.resize(self.width(), 80)
    ...
    def show_slide(self, index):
        ...
        if 0 <= index < len(self.slides):
            ...
            bg_color = QColor(slide.bg_color)
            # Calculate luminance
            luminance = (0.299 * bg_color.red() + 0.587 * bg_color.green() + 0.114 * bg_color.blue()) / 255
            text_color = "black" if luminance > 0.5 else "white"
            
            self.slide_display.setStyleSheet(f"background: {slide.bg_color};")
            self.title_label.setStyleSheet(f"font-size: 48px; font-weight: bold; color: {text_color};")
            self.title_label.setText(slide.title)
            self.content_label.setStyleSheet(f"font-size: 32px; color: {text_color};")
            ...
            if slide.image_path and Path(slide.image_path).exists():
                ...
                pixmap = QPixmap(slide.image_path)
                if not pixmap.isNull():
                    # Scale image to fit nicely in the slide (max 600x400)
...
class Presentation(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.slide_previews = []
        self.updating_ui = True  # Start as True to prevent saving during initialization
        self.setup_ui()
        self.setup_shortcuts()
        self.load_slide(0)
        self.updating_ui = False  # Now ready for user edits
    ...
    def create_toolbar(self):
        ...
        # Image button
        img_btn = self.create_tool_button("🖼️ Image", self.add_image)
    ...
    def load_slide(self, index):
        ...
        if 0 <= index < len(self.slides):
            # Save current slide before switching (but not if we're already on this slide)
            if index != self.current_slide_index:
                self.save_current_slide()
            
            # Set flag to prevent triggering on_content_changed during load
    ...
    def save_current_slide(self):
        ...
        if 0 <= self.current_slide_index < len(self.slides) and not self.updating_ui:
    ...
    def on_content_changed(self):
        ...
        # Only mark as modified and save if not updating UI programmatically
    ...
    def add_image(self):
        ...
        if file_path:
            self.slides[self.current_slide_index].image_path = file_path
            ...
            if not pixmap.isNull():
                ...
                self.image_preview.setPixmap(scaled_pixmap)
                self.is_modified = True
                self.status_label.setText(f"Image added: {Path(file_path).name}")
                play_sound("click.wav")
            else:
    ...
    def new_presentation(self):
        ...
        self.updating_ui = True  # Prevent saving during load
        self.refresh_slide_list()
        self.load_slide(0)
        self.updating_ui = False  # Ready for edits
        play_sound("click.wav")
    ...
    def open_presentation(self):
        ...
        if file_path:
            try:
                ...
                self.load_slide(0)
                self.updating_ui = False  # Ready for edits
                self.status_label.setText("Presentation opened successfully")
    ...
    def save_as_presentation(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Presentation", "presentation.json",
            "JSON Files (*.json);;All Files (*)"
        )
